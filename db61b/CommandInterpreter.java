...
class CommandInterpreter {
     ...
     *  resulting table. Collaborator: Erica*/
    Table selectClause() {
        Table table;
        ArrayList coltitles = new ArrayList();

        _input.nextIf("select");
        ...
        if (_input.nextIf("where")) {
            if (secondt) {
                conditions = conditionClause(t1, t2);
            } else {
                conditions = conditionClause(t1);
            }
        }

        if (!secondt) {
            table = t1.select(coltitles, conditions);
        } else {
            table = t1.select(t2, coltitles, conditions);
        }
        return table;
    }
    ...
    Condition condition(Table... tables) {
        String col1n = _input.next();
        String relation = _input.next();
        Column col1 = new Column(col1n, tables);

        if (_input.nextIs(Tokenizer.LITERAL)) {
            return new Condition(col1, relation, literal());
        } else {
            String col2n = _input.next();
            Column col2 = new Column(col2n, tables);
